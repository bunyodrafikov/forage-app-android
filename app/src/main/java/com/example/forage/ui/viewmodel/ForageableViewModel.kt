...
class ForageableViewModel(
    ...
    val allForages: LiveData<List<Forageable>> = forageableDao.getAll().asLiveData()

    fun getForage(id: Long): LiveData<Forageable> {
        return forageableDao.getElementById(id).asLiveData()
    }

    private fun insertForage(forageable: Forageable) {
...
class ForageViewModelFactory(private val forageableDao: ForageableDao) : ViewModelProvider.Factory {
